...
class LazyPositionalEncodingMatrix(torch.nn.Module):
    """
    Lazy positional encoding that only computes encoding when needed.
    More memory efficient for smaller sequences.
    """
    ...
    def forward(self, seq_len: int) -> torch.Tensor:
        ...
        pe = torch.zeros(seq_len, seq_len, self.d_model)
        ...
        # Create dimension indices for vectorized computation
        dim_indices = torch.arange(0, self.d_model, 4, dtype=torch.float)
        ...
        for idx, k in enumerate(dim_indices):
            ...
            if k < self.d_model:
                # Position i encoding
                div_term_i = 10000 ** (k / self.d_model)
                pe[:, :, k] = torch.sin(positions_i / div_term_i)
                ...
                if k + 2 < self.d_model:
                    # Position j encoding  
                    div_term_j = 10000 ** ((k + 2) / self.d_model)
                    pe[:, :, k + 2] = torch.sin(positions_j / div_term_j)
                
                if k + 3 < self.d_model:
                    # Relative distance encoding
                    div_term_rel = 10000 ** ((k + 3) / self.d_model)
                    pe[:, :, k + 3] = torch.cos(rel_distances / div_term_rel)
        
        # Cache the result
        self._cache[seq_len] = pe
        return pe
...
def load_ours(
    ...
    **kwargs,
    ...
    """
    Load MLCV model - either standard (fixed input size) or transferable version.
    
    Args:
        mlcv_dim: Output dimension of the collective variable
        input_dim: Input dimension (ignored for transferable version)
        dim_normalization: Whether to apply dimension normalization
        normalization_factor: Factor for dimension normalization  
        transferable: If True, return MLCV_TRANSFERABLE model that works across protein sizes
    ...
    Returns:
        MLCV model (either standard or transferable)
    """
    if transferable:
        ...
        print(f"Loaded standard MLCV model with input_dim={input_dim}, output_dim={mlcv_dim}")
        print(mlcv_model)
...
def load_transferable_mlcv(
    ...
    **kwargs,
    ...
    """
    Load transferable MLCV model that works across different protein sizes.
    ...
    Returns:
        MLCV_TRANSFERABLE model
    """
    mlcv_model = MLCV_TRANSFERABLE(
        input_dim=1,  # Dummy value since it's not used for transferable model
        mlcv_dim=mlcv_dim,
        d_model=d_model,
        d_pair=d_pair,
        n_head=n_head,
